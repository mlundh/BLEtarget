...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomScoreHdle;                    /**< Score handle */
  uint16_t  CustomXHdle;                  /**< X_position handle */
  uint16_t  CustomYHdle;                  /**< Y_position handle */
...
#define COPY_SCORE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x10,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_X_POSITION_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x11,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_Y_POSITION_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x12,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for Score +
   *                                2 for X_position +
   *                                2 for Y_position +
   *                                1 for X_position configuration descriptor +
   *                                1 for Y_position configuration descriptor +
   *                              = 7
   *
  ...
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             ...
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomScoreHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: Score, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: Score \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomScoreHdle,
                          ...
                          &(CustomContext.CustomXHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : X, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : X \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomScoreHdle,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(CustomContext.CustomYHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : Y, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : Y \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_X:
      ret = aci_gatt_update_char_value(CustomContext.CustomScoreHdle,
                                       CustomContext.CustomXHdle,
                                       0, /* charValOffset */
                                       SizeX, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value X command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value X command\n\r");
      }
    ...
    case CUSTOM_STM_Y:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value Y command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value Y command\n\r");
      }
